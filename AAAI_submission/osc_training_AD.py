...
def create_optimized_data_loaders(train_subset=None, test_dataset=None, full_train_dataset=None, 
    ...
    Args:
        ...
        batch_size: Base batch size (will be optimized based on available memory)
        device: Target device ('cuda' or 'cpu')
    
    Returns:
        List of configured DataLoaders
    """
    # Optimize batch size based on device and memory
    if torch.cuda.is_available() and device == 'cuda':
        batch_size = 2048  # Aggressive batch sizing for GPU
        num_workers = 6
        pin_memory = True
        prefetch_factor = 3
    ...
    loaders = []
    loader_configs = [
        (train_subset, True),      # Training loader with shuffle
        (test_dataset, False),     # Test loader without shuffle
        (full_train_dataset, True) # Full training loader with shuffle
    ]
...
def extract_latent_vectors(pipeline, loader) -> Tuple[np.ndarray, np.ndarray]:
    ...
    Args:
        pipeline: Trained GATPipeline instance
        loader: DataLoader containing graph data
...
def main(config: DictConfig):
    ...
    # Create normal-only subset for autoencoder training

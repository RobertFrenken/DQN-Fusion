...
class OSCJobManager:
    ...
    def create_slurm_script(
        ...
        # Extract components from config_name (not preset_name) for organized directory structure
        # config_name format: automotive_{dataset}_{supervised/unsupervised}_{model}_{size}_{distill}_{mode}
        # preset_name is simplified, so we parse the original config_name to get full details
        parse_source = config_name.lower()
        dataset_name = None
        model_type = None
        model_size = None
        distill_status = None
        ...
        # Parse dataset (check both with and without underscores)
        for ds in ['hcrl_sa', 'hcrl_ch', 'set_01', 'set_02', 'set_03', 'set_04']:
            if ds in parse_source or ds.replace('_', '') in parse_source:
                dataset_name = ds
                break

        # Parse model type (case-insensitive)
        for model in ['vgae', 'gat', 'dqn']:
            if model in parse_source:
                model_type = model
                break

        # Parse model size
        if 'teacher' in parse_source:
            model_size = 'teacher'
        elif 'student' in parse_source:
            model_size = 'student'

        # Parse distillation status
        if '_yes_' in parse_source or parse_source.endswith('_yes'):
            distill_status = 'with-kd'
        elif '_no_' in parse_source or parse_source.endswith('_no'):
            distill_status = 'no-kd'

        # Parse training mode (check config_name for detailed mode info)
        if 'curriculum' in parse_source:
            training_mode = 'curriculum'
        elif 'fusion' in parse_source:
            training_mode = 'fusion'
        elif 'all_samples' in parse_source or 'autoencoder' in parse_source:
            training_mode = 'autoencoder'
        elif 'normal' in parse_source:
            training_mode = 'normal'
        elif 'distill' in parse_source:
            training_mode = 'distillation'
        else:
            # Fallback: try to infer from preset_name
            if 'autoencoder' in preset_name:
                training_mode = 'autoencoder'
            elif 'curriculum' in preset_name:
                training_mode = 'curriculum'
            elif 'fusion' in preset_name:
                training_mode = 'fusion'
            elif 'distill' in preset_name:
                training_mode = 'distillation'
            elif 'gat_normal' in preset_name:
                training_mode = 'normal'
            else:
                training_mode = 'other'

        # Create hierarchical log directory structure
        # Format: slurm_runs/{dataset}/{model}/{size}/{distill}/{mode}/
        log_path_parts = ["slurm_runs"]

        if dataset_name:
            log_path_parts.append(dataset_name)
        else:
            log_path_parts.append("unknown-dataset")

        if model_type:
            log_path_parts.append(model_type)

        if model_size:
            log_path_parts.append(model_size)

        if distill_status:
            log_path_parts.append(distill_status)

        if training_mode:
            log_path_parts.append(training_mode)

        # Build the full path
        log_subdir = experiment_dir
        for part in log_path_parts:
            log_subdir = log_subdir / part
        ...
        # Human-readable timestamp: YYYY-MM-DD_HH-MM-SS instead of YYYYMMDD_HHMMSS
        timestamp = datetime.now().strftime('%Y-%m-%d_%H-%M-%S')

        # Better filename format: {model}_{size}_{mode}_{dataset}_{timestamp}
        filename_parts = []
        if model_type:
            filename_parts.append(model_type)
        if model_size:
            filename_parts.append(model_size)
        if training_mode and training_mode != 'other':
            filename_parts.append(training_mode)
        if dataset_name:
        ...
        if filename_parts:
            ...
            filename_base = f"{preset_name}_{timestamp}"

        log_file = log_subdir / f"{filename_base}.log"
        error_file = log_subdir / f"{filename_base}.err"

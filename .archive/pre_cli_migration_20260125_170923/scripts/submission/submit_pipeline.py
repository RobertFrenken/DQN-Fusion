...
# Artifact dependencies per mode
ARTIFACT_REQUIREMENTS = {
    "autoencoder": [],  # No dependencies
    "curriculum": ["vgae"],  # Requires VGAE model
    "fusion": ["vgae", "gat"],  # Requires both VGAE and GAT
    "knowledge_distillation": ["teacher"],  # Requires teacher model
    "normal": [],  # No dependencies
}
...
class PipelineSubmitter:
    ...
    def __init__(self, dry_run: bool = False, skip_artifact_check: bool = False):
        self.dry_run = dry_run
        self.skip_artifact_check = skip_artifact_check
        self.project_root = PROJECT_ROOT
    
    def _detect_mode_from_preset(self, preset: str) -> str:
        """Detect training mode from preset name."""
        preset_lower = preset.lower()
        if 'autoencoder' in preset_lower or 'vgae' in preset_lower:
            return 'autoencoder'
        elif 'curriculum' in preset_lower:
            return 'curriculum'
        elif 'fusion' in preset_lower:
            return 'fusion'
        elif 'distill' in preset_lower:
            return 'knowledge_distillation'
        elif 'normal' in preset_lower or 'gat' in preset_lower:
            return 'normal'
        return 'normal'
    
    def _get_artifact_path(self, artifact_type: str, dataset: str) -> Path:
        """Get artifact path for a given type and dataset using glob-based discovery.
        
        This method first tries to discover existing models using pattern matching,
        then falls back to the canonical expected path if nothing is found.
        """
        import sys
        sys.path.insert(0, str(self.project_root / "src"))
        from paths import PathResolver
        
        experiment_root = self.project_root / "experiment_runs" / "automotive" / dataset
        resolver = PathResolver(None)
        
        # Map artifact types to their canonical directories and model types
        artifact_config = {
            "vgae": (
                experiment_root / "unsupervised" / "vgae" / "teacher" / "no_distillation" / "autoencoder",
                'vgae',
                "models/vgae_autoencoder.pth"
            ),
            "gat": (
                experiment_root / "supervised" / "gat" / "teacher" / "no_distillation" / "curriculum",
                'gat',
                "models/gat_curriculum.pth"
            ),
            "teacher": (
                experiment_root / "supervised" / "gat" / "teacher" / "no_distillation" / "fusion",
                'dqn',
                "models/dqn_fusion.pth"
            ),
        }
        ...
        base_dir, model_type, fallback_subpath = artifact_config[artifact_type]
        
        # Try discovery first
        discovered = resolver.discover_model(base_dir, model_type, require_exists=False)
        if discovered:
            return discovered
        
        # Fallback to canonical path
        return base_dir / fallback_subpath
    
    def validate_pipeline_artifacts(self, stages: List[Dict], dataset: str,
                                    prior_stages_in_pipeline: Optional[List[str]] = None) -> Dict:
        """
        Validate that required artifacts exist or will be created by prior stages.
        
        Args:
            stages: List of stage configurations with 'preset' key
            dataset: Dataset name
            prior_stages_in_pipeline: Modes that will run before this stage (creates artifacts)
            
        Returns:
            Dict with 'valid' bool and 'errors' list
        """
        if self.skip_artifact_check:
            return {"valid": True, "errors": [], "warnings": []}
        
        errors = []
        warnings = []
        
        # Track what artifacts prior stages will create
        will_be_created = set()
        if prior_stages_in_pipeline:
            for mode in prior_stages_in_pipeline:
                if mode == 'autoencoder':
                    will_be_created.add('vgae')
                elif mode in ['curriculum', 'normal']:
                    will_be_created.add('gat')
                elif mode == 'fusion':
                    will_be_created.add('teacher')
        
        for i, stage in enumerate(stages):
            preset = stage.get("preset", "").format(dataset=dataset)
            mode = self._detect_mode_from_preset(preset)
            required_artifacts = ARTIFACT_REQUIREMENTS.get(mode, [])
            
            for artifact in required_artifacts:
                # Check if a prior stage in this pipeline creates it
                if artifact in will_be_created:
                    continue
                    
                # Check if it exists on disk
                artifact_path = self._get_artifact_path(artifact, dataset)
                if not artifact_path.exists():
                    errors.append(
                        f"Stage {i+1} ({preset}) requires '{artifact}' artifact at:\n"
                        f"    {artifact_path}\n"
                        f"    This file does not exist and no prior stage creates it."
                    )
            
            # Track what this stage will create for subsequent stages
            if mode == 'autoencoder':
                will_be_created.add('vgae')
            elif mode in ['curriculum', 'normal']:
                will_be_created.add('gat')
    ...
    def submit_pipeline(self, pipeline_name: str, dataset: str,
        ...
        logger.info("🔍 Validating artifact dependencies...")
        validation = self.validate_pipeline_artifacts(stages, dataset)
        if not validation["valid"]:
            ...
            for error in validation["errors"]:
                logger.error(f"\n{error}")
            logger.error("")
            logger.error("To skip this check (not recommended), use --skip-artifact-check")
            logger.error("=" * 80)
            return {"status": "failed", "error": "Artifact validation failed", "details": validation["errors"]}
        logger.info("   ✅ All required artifacts exist or will be created by prior stages")
        logger.info("")
...
Examples:
  ...
  python scripts/submit_pipeline.py --pipeline teacher --dataset hcrl_sa --dry-run
    ...
    parser.add_argument("--skip-artifact-check", action="store_true",
                       help="Skip pre-flight artifact validation (not recommended)")
    ...
    # Check required arguments for non-list mode
    if not args.pipeline or not args.dataset:
        parser.error("the following arguments are required: --pipeline, --dataset")

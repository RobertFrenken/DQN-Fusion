...
const Charts = (() => {
    ...
    function noData(container, message) {
        d3.select(container).html(`<div class="no-data">${message || 'No data available'}</div>`);
    }
    ...
    function datasetComparison(container, data, metric) {
        try {
                    ...
                    svg.append('rect')
                        ...
                        .on('mouseover', (event) => {
                            tooltip.style('opacity', 1)
                                .html(`${cfg}<br>${ds}: ${d.best_value != null ? d.best_value.toFixed(4) : '\u2014'}`);
                        })
                        .on('mousemove', (event) => {
            ...
            console.error('Dataset comparison render error:', e);
            noData(container, 'Render error');
        }
    }
    ...
    function kdTransfer(container, data) {
        try {
            ...
            console.error('KD transfer render error:', e);
            noData(container, 'Render error');
        }
    }
    ...
    function trainingCurves(container, data, metric) {
        try {
            ...
            console.error('Training curves render error:', e);
            noData(container, 'Render error');
        }
    }
    ...
    function runTimeline(container, data) {
        try {
            ...
            console.error('Run timeline render error:', e);
            noData(container, 'Render error');
        }
    }
